...
contains
  ...
  subroutine do_with_array_ad(n, m, x, x_ad, y, y_ad, z_ad)
    ...
    ary_save_90_ad(1:n,1:m) = ary(1:n,1:m)
    do j = 1, m
      do i = 1, n
        z(i,j) = ary(i,j) * x(i,j)
    ...
    do j = m, 1, - 1
      do i = n, 1, - 1
        z_save_93_ad = z(i,j)
        ary_save_94_ad = ary(i,j)
        ary_ad(i,j) = z_ad(i,j) ! z(i,j) = z(i,j) + ary(i,j)
        ary(i,j) = ary_save_94_ad
    ...
    ary(1:n,1:m) = ary_save_90_ad(1:n,1:m)
    do j = m, 1, - 1
      do i = n, 1, - 1
        z(i,j) = ary(i,j) * x(i,j)
  ...
  subroutine do_with_local_array_ad(n, m, x, x_ad, y, y_ad, z_ad)
    ...
    do j = m, 1, - 1
      do i = n, 1, - 1
        ...
        work1_save_132_ad(1:2) = work1(1:2,i,j)
        do k = 1, 2
          work3(k) = work1(k,i,j)
        ...
        work1(1:2,i,j) = work1_save_132_ad(1:2)
        do k = 2, 1, - 1
          work3(k) = work1(k,i,j)
