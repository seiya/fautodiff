  ...
  subroutine do_with_array_ad(n, m, x, x_ad, y, y_ad, z_ad)
    ...
    do j = m, 1, -1
       do i = n, 1, -1
          ...
          scalar_ad = 0.0
          ary(i,j) = ary_save_1_ad
          x_ad(i,j) = ary_ad(i,j) * ary(i,j) + x_ad(i,j) ! ary(i,j) = x(i,j) * ary(i,j) + z(i,j)
          ary_ad(i,j) = ary_ad(i,j) * x(i,j) + ary_ad(i,j) ! ary(i,j) = x(i,j) * ary(i,j) + z(i,j)
