...
contains
  ...
  subroutine stencil_loop_with_halo_rev_ad(is, ie, istart, iend, h, h_ad, u, u_ad, dhdt_ad)
    ...
    !$omp parallel do private(flux_ad, flux_ad_n2, flux_ad_n1, flux_ad_p1, flux_ad_p2)
    do i = iend + 2, istart - 2, - 1
      if (i - 2 >= istart) then
        flux_ad_n2(1) = - dhdt_ad(i - 2) ! dhdt(i) = - (flux(1) - flux(2))
      end if
      if (i - 1 >= istart .and. i - 1 <= iend) then
        flux_ad_n1(1) = - dhdt_ad(i - 1) ! dhdt(i) = - (flux(1) - flux(2))
      end if
      if (i >= istart .and. i <= iend) then
        flux_ad(1) = - dhdt_ad(i) ! dhdt(i) = - (flux(1) - flux(2))
      end if
      if (i + 1 >= istart .and. i + 1 <= iend) then
        flux_ad_p1(1) = - dhdt_ad(i + 1) ! dhdt(i) = - (flux(1) - flux(2))
      end if
      if (i - 1 >= istart .and. i - 1 <= iend) then
        flux_ad_n1(2) = dhdt_ad(i - 1) ! dhdt(i) = - (flux(1) - flux(2))
      end if
      if (i >= istart .and. i <= iend) then
        flux_ad(2) = dhdt_ad(i) ! dhdt(i) = - (flux(1) - flux(2))
      end if
      if (i + 1 >= istart .and. i + 1 <= iend) then
        flux_ad_p1(2) = dhdt_ad(i + 1) ! dhdt(i) = - (flux(1) - flux(2))
      end if
      if (i + 2 <= iend) then
        flux_ad_p2(2) = dhdt_ad(i + 2) ! dhdt(i) = - (flux(1) - flux(2))
      end if
      if (i + 1 >= istart .and. i + 1 <= iend) then
        u_ad(i) = flux_ad_p1(2) * (- h(i + 2) + 2.0 * h(i + 1) - 2.0 * h(i) + h(i - 1)) / 6.0 + u_ad(i)
          ! flux(2) = u(i-1) * (- h(i+1) + 2.0 * h(i) - 2.0 * h(i-1) + h(i-2)) / 6.0
      end if
      if (i - 1 >= istart .and. i - 1 <= iend) then
        h_ad(i) = - flux_ad_n1(2) * u(i - 2) / 6.0 + h_ad(i)
          ! flux(2) = u(i-1) * (- h(i+1) + 2.0 * h(i) - 2.0 * h(i-1) + h(i-2)) / 6.0
      end if
      if (i >= istart .and. i <= iend) then
      ...
      if (i + 1 >= istart .and. i + 1 <= iend) then
        h_ad(i) = - flux_ad_p1(2) * u(i) * 2.0 / 6.0 + h_ad(i)
          ! flux(2) = u(i-1) * (- h(i+1) + 2.0 * h(i) - 2.0 * h(i-1) + h(i-2)) / 6.0
      end if
      if (i + 2 <= iend) then
        h_ad(i) = flux_ad_p2(2) * u(i + 1) / 6.0 + h_ad(i)
          ! flux(2) = u(i-1) * (- h(i+1) + 2.0 * h(i) - 2.0 * h(i-1) + h(i-2)) / 6.0
      end if
      if (i >= istart .and. i <= iend) then
      ...
      if (i - 2 >= istart) then
        h_ad(i) = - flux_ad_n2(1) * u(i - 2) / 6.0 + h_ad(i)
          ! flux(1) = u(i) * (- h(i+2) + 2.0 * h(i+1) - 2.0 * h(i) + h(i-1)) / 6.0
      end if
      if (i - 1 >= istart .and. i - 1 <= iend) then
        h_ad(i) = flux_ad_n1(1) * u(i - 1) * 2.0 / 6.0 + h_ad(i)
          ! flux(1) = u(i) * (- h(i+2) + 2.0 * h(i+1) - 2.0 * h(i) + h(i-1)) / 6.0
      end if
      if (i >= istart .and. i <= iend) then
      ...
      if (i + 1 >= istart .and. i + 1 <= iend) then
        h_ad(i) = flux_ad_p1(1) * u(i + 1) / 6.0 + h_ad(i) ! flux(1) = u(i) * (- h(i+2) + 2.0 * h(i+1) - 2.0 * h(i) + h(i-1)) / 6.0
      end if
    end do
    !$omp end parallel do
    !$omp parallel do
