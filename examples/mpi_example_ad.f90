...
contains
  ...
  subroutine isend_irecv_alloc_fwd_ad(x, x_ad, y, y_ad, comm)
    ...
    pp = rank + 1
    if (pn >= 0) then
      call MPI_Irecv_fwd_ad(z(1), z_ad(1), 1, MPI_REAL, pn, tag, comm, reqs(1), reqs_ad(1), ierr) ! call MPI_Irecv(z(1), 1, MPI_REAL, pn, tag, comm, reqs(1), ierr)
      call MPI_Isend_fwd_ad(z(2), z_ad(2), 1, MPI_REAL, pn, tag + 1, comm, reqs(2), reqs_ad(2), ierr) ! call MPI_Isend(z(2), 1, MPI_REAL, pn, tag+1, comm, reqs(2), ierr)
    end if
    if (pp < size) then
      call MPI_Irecv_fwd_ad(z(3), z_ad(3), 1, MPI_REAL, pp, tag + 1, comm, reqs(3), reqs_ad(3), ierr) ! call MPI_Irecv(z(3), 1, MPI_REAL, pp, tag+1, comm, reqs(3), ierr)
      call MPI_Isend_fwd_ad(z(2), z_ad(2), 1, MPI_REAL, pp, tag, comm, reqs(4), reqs_ad(4), ierr) ! call MPI_Isend(z(2), 1, MPI_REAL, pp, tag, comm, reqs(4), ierr)
    end if
  ...
  subroutine isend_irecv_alloc_rev_ad(x_ad, y_ad, comm)
    ...
    y_ad = 0.0 ! y = x + z(2)
    if (pp < size) then
      call MPI_Isend_rev_ad(z_ad(2), 1, MPI_REAL, pp, tag, comm, reqs_ad(4), ierr) ! call MPI_Isend(z(2), 1, MPI_REAL, pp, tag, comm, reqs(4), ierr)
      call MPI_Irecv_rev_ad(z_ad(3), 1, MPI_REAL, pp, tag + 1, comm, reqs_ad(3), ierr) ! call MPI_Irecv(z(3), 1, MPI_REAL, pp, tag+1, comm, reqs(3), ierr)
    end if
    if (pn >= 0) then
      call MPI_Isend_rev_ad(z_ad(2), 1, MPI_REAL, pn, tag + 1, comm, reqs_ad(2), ierr) ! call MPI_Isend(z(2), 1, MPI_REAL, pn, tag+1, comm, reqs(2), ierr)
      call MPI_Irecv_rev_ad(z_ad(1), 1, MPI_REAL, pn, tag, comm, reqs_ad(1), ierr) ! call MPI_Irecv(z(1), 1, MPI_REAL, pn, tag, comm, reqs(1), ierr)
    end if
