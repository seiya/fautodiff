...
contains
  ...
  subroutine call_fucntion_fwd_ad(x_ad, y, y_ad)
    ...
    call bar_fwd_ad(y, y_ad, bar0_save_34_ad) ! x = bar(y)
    x_ad = bar0_save_34_ad ! x = bar(y)
  ...
  subroutine arg_operation_fwd_ad(x, x_ad, y, y_ad)
    ...
    foo_arg1_save_42_ad = y_ad * 2.0 ! call foo(x, y * 2.0)
    call foo_fwd_ad(x, x_ad, y * 2.0, foo_arg1_save_42_ad) ! call foo(x, y * 2.0)
  ...
  subroutine arg_function_fwd_ad(x, x_ad, y, y_ad)
    ...
    call bar_fwd_ad(y, y_ad, bar0_save_50_ad) ! call foo(x, bar(y))
    foo_arg1_save_50_ad = bar0_save_50_ad ! call foo(x, bar(y))
    call foo_fwd_ad(x, x_ad, bar(y), foo_arg1_save_50_ad) ! call foo(x, bar(y))
