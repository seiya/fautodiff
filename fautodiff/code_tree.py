...
class Node:
    ...
    def convert_assignments(self, saved_vars: List[str], func: Callable[[OpVar, Operator, dict], List[Assignment]], reverse: bool = False) -> List["Node"]:
        """New with converted assignment nodes."""
        return []

    def _save_vars(self, var: OpVar, saved_vars: List[str]) -> SaveAssignment:
        id = self.get_id()
...
class Routine(Node):
    ...
    def convert_assignments(self, saved_vars: List[str], func: Callable[[OpVar, Operator, dict], List[Assignment]], reverse: bool = False) -> List[Routine]:
...
class Assignment(Node):
    ...
    def convert_assignments(self, saved_vars: List[str], func: Callable[[OpVar, Operator, dict], List[Assignment]], reverse=False) -> List[Assignment]:
...
class SaveAssignment(Node):
    ...
    id: int
    load: bool = False
    tmpvar: OpVar = field(init=False, repr=False, default=None)
    ...
    def to_load(self) -> "SaveAssignment":
        return SaveAssignment(self.var, id=self.id, load=True)
...
class DoLoop(DoAbst):
    ...
    def convert_assignments(self, saved_vars: List[str], func: Callable[[OpVar, Operator, dict], List[Assignment]], reverse=False) -> List[Node]:
        ...
        if not new_body.is_effectively_empty():
            #for var in new_body.assigned_vars():
            #    if not var.endswith("_ad"):
            #        var = OpVar(var)
            #        new_body.append(self._save_vars(var, saved_vars))
